-- 0014 — align the role label with the frontend's Role type ('user' | 'admin').
-- The enum was originally created with 'customer'; the app (and its API
-- contract) expects 'user'. Renaming the enum LABEL preserves existing rows.

alter type user_role rename value 'customer' to 'user';

-- reseed_demo_data's body has the old literal baked in — re-issue it.
...
begin
  ...
  insert into public.users (name, email, password_hash, role, email_verified) values
    ...
    ('Demo User', 'user@s2vestis.com', p_user_password_hash, 'user', true);
