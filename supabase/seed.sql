-- S2VESTIS · demo seed  (runs on `supabase db reset`)
-- Mirrors server/src/seed/seedData.js: 8 categories, ~33 products with colour
-- variants / per-size stock / placeholder images, an admin + a demo user, and
-- a couple of coupons.

-- idempotent-ish: wipe app data first (order matters for FKs)
truncate
...
-- (public.slugify is defined in migration 0001)

-- ---- users ---------------------------------------------------------
insert into public.users (name, email, password_hash, role, email_verified) values
  ('Admin', 'admin@s2vestis.com',
   '$2a$10$rt.fgH433Xl4gd8lnmzfdu.fbNqqWirfRIMRNNXspAxgy3AvQJBBW', 'admin', true),
  ('Demo User', 'user@s2vestis.com',
   '$2a$10$dmVzyO/TBiIQdJIP6Mxojedtt6saDAeKC7qsmcPVwQIeoAO5BXOYq', 'customer', true);

-- ---- coupons -----------------------------------------------------
insert into public.coupons (code, description, discount_type, discount_value, min_subtotal, max_discount, is_active)
values
...
declare
  ...
  sz text; s_idx integer; stk integer;
  img_n integer;
begin
  -- categories
...
-- Recompute rating aggregates from any seeded reviews (none yet — keeps the
-- seeded static values in place).
